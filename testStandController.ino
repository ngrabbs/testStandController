...
const int armPin = 43;                   // the master enable physical switch
const int enableRelayPin = 45;
...
// const int fillValveSolenoid = 9;   // fuel tank pressure fill valve
// const int dumpValveSolenoid = 10;   // fuel tank pressure dump valve
...
int enableRelayOUTPUT = 0;

unsigned long armPinDebounceTime = 0;
...
enum systemStates {idle, coldFire, testLogger, testPin, pressureTest, hotFire, dePressure};
...
void setup() {
  ...
  // wait a few seconds so the cli is up then preset the message
  delay(15);
  Serial.println("");
  printOptions();
...
void logger(void) {
  ...
  if (DEBUG) {
    Serial.println(String(
      String(millis()-startTime) + "," + systemStates(systemState) + "," + armPinState
      + "," + fuelButtonState + "," + oxidizerButtonState + "," + igniterButtonState + "," + fuelOUTPUT + "," + oxidizerOUTPUT
      + "," + igniterOUTPUT + "," + fillValveOUTPUT + "," + dumpValveOUTPUT + ",CP:" + getSensorVoltage(chamberPressureSensor)
      + ",FP:" + getSensorVoltage(fuelPressureSensor) + ",FP2:" + getSensorVoltage(fuelPressure2Sensor) + ",OP:" + getSensorVoltage(oxidizerPressureSensor)
    ));
  }
}
...
void loop () {
  ...
  if (systemState == idle) {
    ...
    if (Serial.available() > 0) {
      ...
      Serial.println(request);       // print as an ASCII-encoded decimal
      if(request == "1") {
        Serial.println("User requests logger test!");
        changeSystemState(testLogger);
      } else if (request == "2") {
        Serial.println("User requests coldfire!");
        changeSystemState(coldFire);
      } else if (request == "3") {
        Serial.println("User requests testPin!");
    ...
    changeSystemState(idle);
  } else if (systemState == testLogger) {
    int counter = 0;
    while ( counter < 20 ) {
      logger();
      delay(100);
